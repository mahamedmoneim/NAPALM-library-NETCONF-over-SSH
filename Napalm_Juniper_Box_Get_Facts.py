...
"""
# This is NETCONF over SSH code by NAPALM library used to retrive all information for juniper router, Can be run from windows or Linux machines.
...
junosdevice = driver('10.10.20.55', 'lab', 'lab123')
junosdevice.open()

# we use driver (get_facts) and (get_interfaces) to get all router information by driver name (junos)

junos_output = junosdevice.get_facts()
print (json.dumps(junos_output, indent=4))

junos_output = junosdevice.get_interfaces()
print (json.dumps(junos_output, sort_keys=True, indent=4))
